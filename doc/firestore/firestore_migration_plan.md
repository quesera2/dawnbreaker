...
- [ ] **PR2: `UserRepository` の問い合わせと命令を分ける（挙動は変えない）**
    ...
    - `authStateChanges()` は購読時に現在値を 1 回流す仕様なので、初期値は `getUser()` と合わせて
      2 度届く。`AppUser` の値等価（PR1 で実装済み）がこの 2 度目を吸収する。`.skip(1)` で捨てる方法は
      「必ず即座に流す」前提に依存し、値がずれた場合に本物の変化を落とすので採らない
    - **仕様の確認を実装時に行うこと。** もし現在値が流れてこないなら `getUser()` の値が古くなったときに
      追従できないため、`watchUser()` を `async*` にして冒頭で `yield` する
      （`SettingsRepositoryImpl.watchHomeDisplayMode()` と同じ形。あちらは土台の `StreamController` が
      現在値を再生しないので `yield` が必須になっている）
