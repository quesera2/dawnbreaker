...
書き込むのは**アカウント作成時**（`9:00` ＋ 端末の現在タイムゾーン）と、**通知時刻を変更したとき**だけ。
...
| scheduledAt | timestamp | この `notifyAt` の算出元となった `nextScheduledAt` の値 |
| lastNotifiedFor | timestamp? | 通知済みの `scheduledAt` の値。重複送信の防止に使う |

通知対象のないタスク（通知が無効・`irregular`・未実行）はドキュメント自体を作らない。
